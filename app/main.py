    ...
    try:
        if db.query(Announcement).count() == 0:
            db.add_all(
                [
                    Announcement(
                        title="VideoMatchへようこそ!",
                        body="「マッチング開始」ボタンを押すと、待機中のユーザーからランダムに通話相手が選ばれます。通話前の注意事項を守って楽しくご利用ください。",
                    ),
                    Announcement(
                        title="ベータ版として運用中です",
                        body="現在デモ運用中のため、予告なくデータがリセットされる場合があります。不具合を見つけた場合は掲示板でお知らせください。",
                    ),
                ]
            )
            db.commit()
