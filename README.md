...
- **トラスト&セーフティ**: 通報(管理者・チームリーダーが確認) / ブロック / 警告文のポップアップ /
  パスワードリセット
- **管理**: サイト設定(20項目超) / ユーザー管理(CSV一括登録・メール案内) / 利用レポート(CSVエクスポート) /
...
- 初回起動時に管理者 `administrator` / `password` が自動作成されます(初回ログイン時にパスワード変更が必要)
...
python tests/test_flow.py   # 169項目の結合テスト
```
