...
- **コミュニティ**: お知らせ / イベントカレンダー / 掲示板 / チーム(専用のチーム画面・
  複数リーダー・設定変更・脱退・チーム限定の掲示板/予定) /
  ルーム(カード表示+詳細モーダル・合言葉・定員・期限・通話設定の上書き・カレンダー連携) /
  マイページ(プロフィール・メール/パスワード変更・履歴)
...
python tests/test_flow.py   # 211項目の結合テスト
```
